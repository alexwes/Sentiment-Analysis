...
class TwitterClient():
    ...
    def get_recent_tweets(self, name):
        ...
        tweets = api.search(q= name + self._excluded_terms, count = 10,
                        lang = "en", tweet_mode="extended")
        return self.get_tweet_data(tweets)

    def populate_firebase(self):
        ...
        names = ["cardano" ,"polkadot","uniswap","chainlink"]
        for name in names:
            tweets = self.get_recent_tweets(name)
            fire.add_tweets(tweets, name, classifier)
    ...
    def get_daily_sentiment(self, name):
        ...
        return docs[0].to_dict()['daily-sentiment']
